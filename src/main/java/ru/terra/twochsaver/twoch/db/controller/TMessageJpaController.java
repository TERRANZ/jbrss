 ...
 */
package ru.terra.twochsaver.twoch.db.controller;
...
import ru.terra.twochsaver.twoch.db.controller.exceptions.NonexistentEntityException;
...
public class TMessageJpaController implements Serializable {
	...
	public List<TMessage> findByParent(Long parent) {
		EntityManager em = getEntityManager();
		try {
			Query q = em.createNamedQuery("TMessage.findByParent", TMessage.class).setParameter("parent", parent);
			return q.getResultList();
